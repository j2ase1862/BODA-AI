...
namespace BODA_VISION_AI.Models
{
    ...
    public static class HelpContent
    {
        ...
        private static readonly Dictionary<string, ToolHelp> _toolHelp = new()
        {
            ...
            // Pattern Matching
            ["TemplateMatchTool"] = new ToolHelp
            {
                Name = "Template Match (템플릿 매칭)",
                Description = "학습된 템플릿(패턴) 이미지를 검색 이미지에서 찾습니다.",
                Usage = "부품 위치 검출, 정렬, 유무 검사, 개수 카운팅에 사용됩니다.",
                CognexEquivalent = "CogPMAlignTool (PatMax)",
                Parameters = new Dictionary<string, string>
                {
                    ["MatchThreshold"] = "매칭 점수 임계값 (0-1). 이 값 이상이면 매칭 성공으로 판정.",
                    ["MaxResults"] = "반환할 최대 매칭 결과 수.",
                    ["EnableMultiScale"] = "다중 스케일 검색 활성화. 크기 변화된 객체 검출 가능.",
                    ["MinScale"] = "검색할 최소 스케일 (0.5 = 50%).",
                    ["MaxScale"] = "검색할 최대 스케일 (1.5 = 150%).",
                    ["ScaleStep"] = "스케일 검색 간격.",
                    ["EnableMultiAngle"] = "다중 각도 검색 활성화. 회전된 객체 검출 가능.",
                    ["AngleStep"] = "각도 검색 간격 (도)."
                }
            },

            ["FeatureMatchTool"] = new ToolHelp
            {
                Name = "Feature Match (특징점 매칭)",
                Description = "SIFT, ORB 등의 특징점을 추출하여 패턴을 매칭합니다.",
                Usage = "복잡한 패턴, 부분 가림, 조명 변화가 있는 상황에서 사용됩니다.",
                CognexEquivalent = "CogPMAlignTool (PatFlex)",
                Parameters = new Dictionary<string, string>
                {
                    ["DetectorType"] = "특징점 검출기:\n• ORB: 빠름, 회전 불변\n• SIFT: 정확함, 스케일 불변\n• AKAZE: 빠르고 정확함",
                    ["MaxFeatures"] = "추출할 최대 특징점 수.",
                    ["RatioThreshold"] = "Lowe's ratio test 임계값. 낮을수록 엄격한 매칭.",
                    ["MinMatchCount"] = "최소 매칭 특징점 수. 이 수 이상이면 성공.",
                    ["DrawMatches"] = "매칭된 특징점 시각화 여부."
                }
            },
